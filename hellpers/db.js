...
class DB {
  ...
  static async flush (collection) {
    return DB.connect()
      .then((connection) => connection.collection(collection).drop())
      .catch((err) => {});
  }

  static async create (collection, data) {
    return DB.connect()
      .then((connection) => connection.collection(collection).insertOne(data))
      .then((result) => this.findOne(collection, { _id: result.insertedId }));
  }

  static async findOne (collection, query) {
    return DB.connect()
      .then((connection) => connection.collection(collection).findOne(query));
  }
